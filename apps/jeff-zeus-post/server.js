...
function loadJwtSecret() {
  ...
  console.log('[zeus-post] JWT secret gerado em data/.jwt-secret (defina JWT_SECRET no ambiente pra fixar um próprio)');
  return secret;
}
...
function auth(req, res, next) {
  ...
  // header em redirect) e no proxy de mídia (usado em <img>/<video> src, que
  // também não mandam header). Nas rotas de API restantes, apenas o header
  // Authorization — token em URL vaza em logs de acesso e no Referer.
  const isOauthStart = req.method === 'GET' && req.path.startsWith('/oauth/');
  const isMediaProxy = req.method === 'GET' && (req.path.startsWith('/media/drive/') || req.originalUrl?.startsWith('/api/media/drive/'));
  const allowQueryToken = isOauthStart || isMediaProxy;
  const token = req.headers.authorization?.replace('Bearer ', '') || (allowQueryToken ? req.query?.token : null);
  if (!token) {
...
function initAdmin() {
  ...
  if (!user) {
    ...
    console.log('[zeus-post] Admin criado: jeff / Zeus@2026 — TROQUE A SENHA em Configurações');
  } else if (bcrypt.compareSync('Zeus@2026', user.password_hash)) {
    console.warn('[zeus-post] ⚠️ SENHA PADRÃO AINDA EM USO — troque em Configurações > Alterar senha');
  }
}

// Rate limit de login: 10 tentativas por IP a cada 15 min (anti brute-force)
...
app.post('/api/auth/change-password', auth, (req, res) => {
  ...
  if (!bcrypt.compareSync(String(current), user.password_hash)) {
    console.log(`[zeus-post] [change-password] senha atual incorreta para user=${user.username}`);
    return res.status(400).json({ error: 'Senha atual incorreta.' });
  }
  const hash = bcrypt.hashSync(String(newPass), 10);
  db.prepare('UPDATE users SET password_hash = ? WHERE id = ?').run(hash, req.user.id);
  console.log(`[zeus-post] [change-password] senha alterada com sucesso para user=${user.username}`);
  res.json({ ok: true });
});
...
app.post('/api/accounts/import-bm', auth, async (req, res) => {
  try {
...
const META_REDIRECT_URI = 'https://zeus-post.jefersonhenrike.com/oauth/instagram/callback';
app.get('/oauth/instagram/start', auth, (req, res) => {
  ...
  if (configId) {
    ...
    return res.redirect(`https://www.facebook.com/v19.0/dialog/oauth?client_id=${appId}&redirect_uri=${redirectUri}&config_id=${configId}&response_type=code&state=ig_connect${authType}`);
  }
  ...
  res.redirect(`https://www.facebook.com/v19.0/dialog/oauth?client_id=${appId}&redirect_uri=${redirectUri}&scope=${scopes}&response_type=code&state=ig_connect${authType}`);
});
...
app.get('/oauth/drive/start', auth, (req, res) => {
  try {
    const redirectUri = `https://zeus-post.jefersonhenrike.com/oauth/drive/callback`;
    res.redirect(drive.buildAuthUrl(redirectUri));
  } catch (err) {
...
app.get('/oauth/youtube/start', auth, (req, res) => {
  ...
  const redirectUri = encodeURIComponent(`https://zeus-post.jefersonhenrike.com/oauth/youtube/callback`);
...
app.get('/oauth/youtube/callback', async (req, res) => {
  ...
  try {
    ...
    const redirectUri = `https://zeus-post.jefersonhenrike.com/oauth/youtube/callback`;
    const axios = require('axios');
...
app.get('/oauth/tiktok/start', auth, (req, res) => {
  ...
  const redirectUri = encodeURIComponent(`https://zeus-post.jefersonhenrike.com/oauth/tiktok/callback`);
...
app.get('/oauth/tiktok/callback', async (req, res) => {
  ...
  try {
    ...
    const redirectUri = `https://zeus-post.jefersonhenrike.com/oauth/tiktok/callback`;
    const axios = require('axios');
...
app.post('/api/posts', auth, (req, res) => {
  const { title, caption, media_path, media_type, post_type, platforms, account_ids, scheduled_at, is_trial } = req.body;
  ...
  // Trial Reel só faz sentido em vídeo
  const trialFlag = is_trial && (post_type === 'video' || post_type === 'reel') ? 1 : 0;
  const db = getDb();
  const result = db.prepare(`INSERT INTO posts (title, caption, media_path, media_type, post_type, is_trial, platforms, account_ids, scheduled_at, status)
...
app.put('/api/posts/:id', auth, (req, res) => {
  const { title, caption, media_path, media_type, post_type, platforms, account_ids, scheduled_at, status, is_trial } = req.body;
  const trialFlag = is_trial && (post_type === 'video' || post_type === 'reel') ? 1 : 0;
  const db = getDb();
  db.prepare(`UPDATE posts SET title=?, caption=?, media_path=?, media_type=?, post_type=?, is_trial=?, platforms=?, account_ids=?,
...
app.get('/api/analytics', auth, (req, res) => {
  ...
  const summary = analytics.getAnalyticsSummary(db, accountId);
  const heatmap = analytics.getBestHoursHeatmap(db);
  const recentPosts = analytics.getRecentPostsWithMetrics(db, 20, accountId);
...
app.get('/api/accounts/permissions', auth, async (req, res) => {
  try {
    const axios = require('axios');
    const db = getDb();
    const appId = settingsLib.getSetting('meta_app_id');
    ...
    for (const a of accounts) {
      try {
        const dbg = await axios.get('https://graph.facebook.com/v19.0/debug_token', {
          params: { input_token: open(a.access_token), access_token: `${appId}|${appSecret}` }
        });
        ...
        results.push({ id: a.id, username: a.platform_username, error: err.message });
      }
    }
...
app.post('/api/analytics/collect', auth, async (req, res) => {
  try {
...
app.get('/api/diagnostics/meta', auth, async (req, res) => {
  try {
...
app.use((err, req, res, next) => {
  console.error(`[zeus-post] erro não tratado em ${req.method} ${req.path}:`, redact(err?.stack || err));
  if (res.headersSent) return next(err);
  res.status(500).json({ error: redact(err?.response?.data?.error?.message || err?.message || 'Erro interno') });
});

process.on('unhandledRejection', (err) => {
  console.error('[zeus-post] unhandledRejection:', redact(err?.stack || err));
});
process.on('uncaughtException', (err) => {
  console.error('[zeus-post] uncaughtException:', redact(err?.stack || err));
});

app.listen(PORT, '0.0.0.0', () => {
  initAdmin();
  console.log(`[zeus-post] Rodando na porta ${PORT}`);
