...
function initSchema() {
  ...
  // posts: is_trial (Trial Reels — Meta is_trial no container REELS)
  const postCols = db.prepare("PRAGMA table_info(posts)").all().map(c => c.name);
  if (!postCols.includes('is_trial')) {
    db.exec("ALTER TABLE posts ADD COLUMN is_trial INTEGER NOT NULL DEFAULT 0");
  }
  ...
  if (!colNamesAfter.includes('is_trial')) {
    db.exec("ALTER TABLE bulk_campaigns ADD COLUMN is_trial INTEGER NOT NULL DEFAULT 0");
  }
