...
async function publishInstagram(post, account) {
  ...
  if (post.post_type === 'story') {
    ...
    const uploadRes = await axios.post(`https://graph.facebook.com/v22.0/${igId}/media`, params);
    ...
    const publishRes = await axios.post(`https://graph.facebook.com/v22.0/${igId}/media_publish`, {
  ...
  if (post.post_type === 'carousel') {
    ...
    for (const url of urls) {
      ...
      const r = await axios.post(`https://graph.facebook.com/v22.0/${igId}/media`, params);
      const cid = r.data.id;
      if (isVideo) await waitForContainer(cid, token);
      childIds.push(cid);
    }
    const carouselRes = await axios.post(`https://graph.facebook.com/v22.0/${igId}/media`, {
  ...
  if (looksLikeVideo) {
    // Trial Reel: publicação só pra não-seguidores (share_to_feed incompatível com is_trial)
    const trial = !!post.is_trial;
    const reelParams = trial
      ? { media_type: 'REELS', video_url: mediaUrl, caption: post.caption, is_trial: true, access_token: token }
      : { media_type: 'REELS', video_url: mediaUrl, caption: post.caption, share_to_feed: true, access_token: token };
    console.log(`[publisher] post ${post.id} reelParams (sem token):`, JSON.stringify({ ...reelParams, access_token: '***' }));
    const uploadRes = await axios.post(`https://graph.facebook.com/v22.0/${igId}/media`, reelParams);
    const containerId = uploadRes.data.id;
    await waitForContainer(containerId, token);
    const publishRes = await axios.post(`https://graph.facebook.com/v22.0/${igId}/media_publish`, {
      creation_id: containerId, access_token: token
    ...
    const uploadRes = await axios.post(`https://graph.facebook.com/v22.0/${igId}/media`, {
...
async function waitForContainer(containerId, token, maxWait = 600000, pollInterval = 10000) {
  ...
  while (Date.now() - start < maxWait) {
    const res = await axios.get(`https://graph.facebook.com/v22.0/${containerId}`, {
      params: { fields: 'status_code,status', access_token: token }
    });
    lastStatus = res.data;
    if (res.data.status_code === 'FINISHED') return;
    if (res.data.status_code === 'ERROR') {
      const raw = res.data.status || 'sem detalhe';
      // Erro 2207082 é o mais comum em stories: geralmente transient no
      // encoder da Meta (mesma mídia publica ok na próxima tentativa). O
      // retry automático em lib/bulk.js já cobre — só melhora o texto.
      const friendly = /2207082/.test(raw)
        ? `Meta rejeitou o processamento do vídeo (erro 2207082 — normalmente transient no encoder da Meta). Retry automático em 10 min. Se persistir: confirme que o vídeo é MP4 H.264 + AAC, vertical 9:16, < 60s pra story.`
        : raw;
      throw new Error('Container error: ' + friendly);
    }
