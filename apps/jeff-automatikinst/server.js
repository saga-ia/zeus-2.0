...
app.post('/api/auth/change-password', auth, (req, res) => {
  ...
  if (!bcrypt.compareSync(String(current), user.password_hash)) {
    console.log(`[automatikinst] [change-password] senha atual incorreta para user=${user.username}`);
    return res.status(400).json({ error: 'Senha atual incorreta.' });
  }
  const hash = bcrypt.hashSync(String(newPass), 10);
  db.prepare('UPDATE users SET password_hash = ? WHERE id = ?').run(hash, req.user.id);
  console.log(`[automatikinst] [change-password] senha alterada com sucesso para user=${user.username}`);
  res.json({ ok: true });
});
