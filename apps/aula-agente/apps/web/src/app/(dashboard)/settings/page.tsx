...
export default function SettingsPage() {
  ...
  const fetchApiKeys = useCallback(async () => {
    ...
    ((data as { provider: string; encrypted_key: string }[] | null) || []).forEach((s) => {
  ...
  const handleSaveApiKey = async (provider: LLMProvider) => {
    ...
    if (!key) {
      await supabase.from("organization_secrets").delete().eq("organization_id", currentOrg.id).eq("provider", provider);
    } else {
    ...
    // Invalida o cache de chaves nos workers via pubsub.
    try {
      ...
      // não bloqueia o save: cache expira em 60s de qualquer forma
    }
          ...
          {PROVIDERS.map((provider) => (
            <div key={provider.id} className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <Button variant="outline" onClick={() => handleSaveApiKey(provider.id)} disabled={savingKeys}>
                  Salvar
                </Button>
              </div>
            </div>
