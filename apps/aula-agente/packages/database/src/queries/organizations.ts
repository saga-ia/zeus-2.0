...
export async function getOrganizationMembers(client: SupabaseClient, organizationId: string) {
  const { data, error } = await client.from("organization_members").select("*").eq("organization_id", organizationId);
  if (error) throw error;
  return data as OrganizationMember[];
}

export async function getUserOrganizations(client: SupabaseClient, userId: string) {
  const { data, error } = await client.from("organization_members").select("*, organizations(*)").eq("user_id", userId);
  if (error) throw error;
  return data;
}
...
export async function createOrganization(
  ...
  const { data: orgData, error: orgError } = await client.from("organizations").insert(org).select().single();
