...
export async function getAgentsByOrganization(client: SupabaseClient, organizationId: string) {
  const { data, error } = await client.from("agents").select("*").eq("organization_id", organizationId).order("created_at", { ascending: false });
...
export async function createAgent(client: SupabaseClient, agent: Omit<Agent, "id" | "created_at" | "updated_at">) {
...
export async function updateAgent(client: SupabaseClient, id: string, updates: Partial<Agent>) {
  const { data, error } = await client.from("agents").update(updates).eq("id", id).select().single();
  if (error) throw error;
  return data as Agent;
}
